...
-----
#### Anaconda python 버전 downgrade (3.7 -> 3.6)

'20.7월 기준, anaconda를 설치하면 기본적으로 python 3.7 버전이 설치 됩니다.

현재 tensorflow 2.1이 python 3.6 환경에서 안정적으로 동작하기 때문에, tensorflow를 사용하기 위해서는 아래의 명령으로 anaconda의 기본 python 버전을 3.7에서 3.6으로 다운그레이드 해야 합니다.

~~~cmd
conda install python=3.6
~~~

Y/N 확인 메시지가 나오면 y를 입력 후 엔터치면 다운그레이드 됩니다.

Anaconda에서 python 버전을 지정하여 별도의 가상환경을 만드는 방법(`conda create --name <가상환경명칭> python=3.6`)도 있지만, python 및 anaconda 환경 설정에 따라 원하는 대로 설치되지 않을수도 있으니 위의 방법으로 anaconda python 버전을 다운그레이드 하는 것을 추천 합니다.
