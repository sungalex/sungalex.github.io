...
![cuDNN Download](/img/cuDNN-download2.png)
...
Anaconda를 설치하면 기본 개발환경으로 "base"라는 가상환경이 설정되어 있습니다. "base" 가상환경에는 기본적인 numpy, pandas, matplolib 등 Data Science에 필요한 기본적인 Python 패키지들이 설치되어 있습니다.
...
여러 개의 패키지(라이브러리)를 한번에 설치할 수 있으며, Spacebar(공백문자)로 구분하여 여러 개 패키지를 나열할 수 있습니다.
  ...
  conda install numpy pandas scipy matplotlib spyder seaborn scikit-learn
  ```

ipython, notebook, jupyter는 jypyter notebook 관련 패키지들 입니다. numpy, pandas, scipy는 tensorflow 패키지를 설치했다면 이미 함께 설치되었을 것 입니다. matplotlib, seaborn는 그래프(도형)을 화면에 그려주는 패키지이며, scikit-learn는 다양한 머신러닝 알고리즘을 지원하는 패키지 입니다.

## Jupyter Notebook 개발환경 테스트

아래 명령으로 Jupyter Notebook을 실행할 수 있습니다. 위에서 "vane"이라는 Anaconda 가상환경에 Jupyter Notebook을 설치했기 때문에 "vane" 가상환경에서 아래 명령을 실행 해야 합니다. Prompt 맨 앞 부분에 "(vane)"이 나타나 있어야 합니다.

  ```bash
  jupyter notebook
  ```

Jupyter Notebook 사용법은 [여기](https://greeksharifa.github.io/references/2019/01/26/Jupyter-usage/)에 잘 설명되어 있습니다.
